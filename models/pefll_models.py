...
# Based on the original PeFLL implementation https://github.com/jonnyascott/PeFLL/blob/main/models.py
...
class CNNTargetEmb(nn.Module):
    def __init__(self, channels=3, n_kernels=16, **kwargs):
        super(CNNTargetEmb, self).__init__()

        self.conv1 = nn.Conv2d(channels, n_kernels, 5)
        self.norm1 = EmbNormLayer(n_kernels, **kwargs)
        self.pool = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(n_kernels, 2 * n_kernels, 5)
        self.norm2 = EmbNormLayer(2 * n_kernels, **kwargs)
        self.fc1 = nn.Linear(2 * n_kernels * 5 * 5, 120)
        self.norm3 = EmbNormLayer(120, **kwargs)
        self.fc2 = nn.Linear(120, 84)
        self.norm4 = EmbNormLayer(84, **kwargs)
